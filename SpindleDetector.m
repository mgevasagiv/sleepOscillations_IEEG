...
%         function updateSpindleTimes(obj, runData)
%             %go over ripple times in folders and change max point - a one
%             %time thing
%             nPatients = length(runData);
%             for iPatient = 1:nPatients
%                 filesInFolder = dir([runData(iPatient).SpindlesFileNames,'*']);
%                 filesInFolder = {filesInFolder.name};
%                 for iFile = 1:length(filesInFolder)
%                     disp(['updating ',filesInFolder{iFile}]);
%                     
%                     numSp = str2num(filesInFolder{iFile}(13:end-4));
%                     
%                     try
%                         data = [runData(iPatient).DataFolder '\CSC' num2str(numSp) '.mat'];
%                         data = load(data);
%                         data = data.data;
%                     catch
%                         disp([runData(iPatient).DataFolder '\CSC' num2str(numSp) '.mat doesn''t exist']);
%                         continue;
%                     end
%                     
...
%                     %load spindles
%                     try
%                         spindlesTimes = [runData(iPatient).SpindlesFileNames num2str(numSp) '.mat'];
%                         spindlesTimes = load(spindlesTimes);
%                         spindlesTimes = spindlesTimes.spindlesTimes;
%                     catch
%                         disp([runData(iPatient).SpindlesFileNames num2str(numSp) '.mat doesn''t exist']);
%                         spindlesTimes = [];
%                     end
%                     
%                     nSpindles = length(spindlesTimes);
%                     newSpindlesTimes = zeros(1,nSpindles);
%                     for iSpindle = 1:nSpindles
